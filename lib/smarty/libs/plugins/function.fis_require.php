...
class ResourceAPI {
    ...
    public function registerMapPath($path, $module){
        $this->map_path[$module] = $path;
    }
    
    private function loadMap($module){
        $map = &$this->map_path;
        if(isset($map[$module])){
            include($map[$module]);
            return true;
    ...
    public function addSymbol($id, $module){
        ...
        if($info['is_lib']){
            ...
            if((isset($this->map['common']) || $this->loadMap('common')) && isset($this->map['common']['resource'][$id])){
                $map = &$this->map['common'];
                $info = &$map['resource'][$id];
            }
        }
        foreach($info['requires'] as &$require){
            $this->addSymbol($require[0], $require[1]);
        }
        $ext = $info['ext'];
    ...
    public function registerResourceMap($map, $module) {
        $this->map[$module] = &$map;
    }
...
function smarty_function_fis_require($params, Smarty_Internal_Template $template){
    ...
    if(isset($params['module'])){
        ...
        throw new Exception("This params have no module params,when use $static");
    }
    /**@var ResourceAPI $api*/
