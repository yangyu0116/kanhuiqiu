...
class CZcacheAdapter
{
  ...
  {
     $loc = $this->zcfObj['CURRENT_CONF'];
	 $servers = $this->zcfObj['arrServers'][$loc];
     if(count($servers)===3){ //for compatible
         $locs = array('jx'=>'tc','tc'=>'jx'); 
         $last_idx = null;
         $retry =  0;
         while ( $retry < 2 ) //retry twince
         {            
            if($retry){
                $server = $this->zcfObj['arrServers'][$locs[$loc]];	
            }else{
                $server = $servers;	
            }
            ...
            if (!isset($this->handlers[$key]))
            {
                ...
                if ($socket->connect($this->zcfObj['CONNTIMEOUT']))
                {
                   ...
                   $retry++;
				   CLogger::warning("connect to zcache server($key) failed,retrying");
                }
            }
            else
  ...
  protected function putHandler($handler, $force = 1)
  {
  ...
  protected function readResponse($handler)
  {
	 ...
	 {
		$this->last_err_no=-12;
		$this->last_err="net err, handler null readResponse";
		return NULL;
	 }
	 ...
	 {
		$this->last_err_no=-12;
		$this->last_err="net err, read fail readResponse";
		return NULL;
	 }
  ...
  { 
    ...
    {
		$this->last_err_no=-12;
		$this->last_err="net err, addOne, handler null";
		return $this->last_err_no;
        }
	 ...
	 if (!$handler->write($buffer))
        {
		$this->last_err_no=-12;
		$this->last_err="net err, addOne, handler write fail";
  ...
  public function updateOne($key_str,$subkey_str,$value_object,$delaytime_int=0,$pid_int=0,$logid_int=0)
  {
	 ...
	 if (!$handler)
    {
	 ...
	 if (!$handler->write($buffer))
        {
		$this->last_err_no=-12;
		$this->last_err="handler write fail";
		$this->putHandler($handler, 1);
		return $this->last_err_no;
        }
  ...
  public function getOne($key_str,$subkey_str,$pid_int=0,$logid_int=0)
  {
	 ...
	 if (!$handler)
    {
	 ...
	 if (!$retbuffer)
        {
			CLogger::warning("addOne readResponse fail");
			$this->putHandler($handler, 1);
			return NULL;
        }
...
public function getMultiple($key_arr,$pid_int=0,$logid_int=0)
  ...
  {  
    ...
    {
		$this->last_err_no=-12;
		$this->last_err="net err, deleteOne, handler null";
		return $this->last_err_no;
        }
	 ...
	 if (!$handler->write($buffer))
        {
		$this->last_err_no=-12;
		$this->last_err="net err, deleteOne, handler write fail";
		$this->putHandler($handler, 1);
		return $this->last_err_no;
        }
